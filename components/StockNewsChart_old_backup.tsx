  ...
  const filteredChartData = chartData.map((point) => ({
    ...
    news: point.news.filter((article) => {
      ...
      const keywordsMatch = selectedKeywords.length === 0 ||
        JSON.parse(article.keywords as any).some((k: string) => selectedKeywords.includes(k))

      return sourceMatch && entityMatch && sentimentMatch && peopleMatch && orgMatch && keywordsMatch
    })
  }))
